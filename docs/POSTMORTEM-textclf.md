...
renumbered *per request*, so `PII_PERSON_N` and `PII_PERSON_M` are the same entity in two rows.
Raw un-redacted identifiers also survive inside the memory content (`REDACTED_VENDOR_ID_2`, `REDACTED_VENDOR_ID_3`,
`REDACTED_VENDOR_ID_1`). Any model fed the raw text memorises rows instead of learning the task.
