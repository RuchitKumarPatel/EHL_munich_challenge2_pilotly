...
def main() -> None:
    ...
    for trajectory in load_trajectories(source):
        quality, quality_type = calibrated_outcome(trajectory, ground_truth)
        rows.append({"trajectory": trajectory.key, "model": trajectory.logged_model, "quality": quality, "signals": trajectory_signals(trajectory), "quality_type": quality_type})
