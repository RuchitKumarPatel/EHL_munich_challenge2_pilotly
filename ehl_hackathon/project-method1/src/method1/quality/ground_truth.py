...
def load_ground_truth(source: str | Path) -> dict[str, float]:
    ...
    by the same SHA-256 trajectory key the loader derives. Looks next to `source`
    (as a dir) and in its parent (source == an `export/` dir, manifest sits beside it).
    Returns {} when no manifest exists (e.g. the organizer's raw export), which is a
    no-op fallback to the existing structural proxy everywhere this is used."""
    path = Path(source)
